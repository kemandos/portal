...
function App() {
  ...
  const toggleTheme = () => {
      const newMode = currentThemeSettings.mode === 'light' ? 'dark' : 'light';
      setPeopleThemeSettings(p => ({...p, mode: newMode}));
      setProjectThemeSettings(p => ({...p, mode: newMode}));
  };
  ...
  // Modals for Settings
  const [isAppSettingsOpen, setIsAppSettingsOpen] = useState(false); // Global (Sidebar)
  const [isHeatmapSettingsOpen, setIsHeatmapSettingsOpen] = useState(false); // Heatmap Specific (Header)
    ...
    const filteredItems = allItems.filter(item => {
        ...
        return activeFilters.every(filter => {
            ...
            if (filter.key === 'Capacity') {
                ...
                const isOverbooked = avgUtil > settings.thresholds!.over; 
            ...
            if (filter.key === 'Utilization') return true; 
  ...
  const commonProps = {
    ...
    // View Layouts use this to trigger Heatmap Settings (view-specific)
    onOpenSettings: () => setIsHeatmapSettingsOpen(true),
  ...
  return (
    <div className="flex h-screen w-full bg-[#F5F2EB] dark:bg-[#0c0c0e] transition-colors duration-300 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
          ...
          {showSelectionBar && (
            <div className={`fixed left-1/2 transform -translate-x-1/2 z-50 flex items-center justify-center w-full px-4 pointer-events-none ${selectionBarClasses} transition-all duration-300`}>
                <div className="bg-[#1e293b] text-white rounded-2xl shadow-[0_8px_40px_-12px_rgba(0,0,0,0.5)] p-2 pr-4 flex items-center gap-6 animate-in slide-in-from-bottom-5 fade-in duration-300 border border-white/10 ring-1 ring-black/20 pointer-events-auto backdrop-blur-xl max-w-fit">
                    <div className="flex items-center gap-4 pl-2">
                        ...
                        <div className="flex flex-col gap-0.5 min-w-[100px]">
                            <span className="text-sm font-bold text-white leading-none truncate max-w-[200px]">
                                {selectionLabel}
                            </span>
                    ...
                    <div className="flex items-center gap-3">
                        <button 
                            ...
                            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white transition-all text-xs font-bold active:scale-95 border border-white/5 hover:border-white/20 hover:shadow-lg shadow-black/20 whitespace-nowrap"
                        >
      ...
      {/* Heatmap Settings Modal (View Header) */}
